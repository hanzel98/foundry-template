### Challenge Solution QuillCTF - Donate

#### Description

The function abi.encodeWithSignature receives a function signature but internally the contract only stored the function selector of the functions, which means that the function signature is parsed to a function selector.
Since the function selector includes only the first 4 bytes of the hash of the signature it is possible to have multiple function signatures that produce the same function selector.

#### Attack steps

We could send any string and try to get the same function selector changeKeeper(address) (0x09779838). We would need some script to generate random inputs with hashing text to the the same (0x09779838)
But fortunately for this case it was possible to use an Ethereum signature database to find a function with the same selector that we needed. Here is the link to the DB
